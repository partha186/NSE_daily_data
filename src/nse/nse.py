...
class NSE:
    ...
    def _get_download_method(self):
        """Get the appropriate iterator method for downloading chunks.
        Returns tuple of (method_name, chunk_method)
        """
        if self.server:
            return ("stream", "iter_bytes")
        return ("get", "iter_content")
    ...
    def _download(self, url: str, folder: Path):
        ...
        method_name, chunk_method = self._get_download_method()
        kwargs = {"timeout": self.timeout}
        
        if method_name == "stream":
            kwargs["url"] = url
            response = self._session.stream("GET", **kwargs)
        else:
        ...
        with response as r:
            ...
            with fname.open(mode="wb") as f:
                iterator = getattr(r, chunk_method)(chunk_size=self.CHUNK_SIZE)
    ...
    def _build_date_params(
        ...
        :param to_date: Optional end date
        :param date_format: Date format string for API
        :return: Dictionary with from_date and to_date keys if both provided
        """
    ...
    def _validate_download(self, file: Path) -> None:
        """Validate downloaded file and raise error if invalid.
        
        :param file: Path to downloaded file
    ...
    def equityBhavcopy(
        ...
        if date.date() < self.UDIFF_SWITCH_DATE:
            ...
            url = f"{self.archive_url}/content/cm/BhavCopy_NSE_CM_0_0_0_{date.strftime('%Y%m%d')}_F_0000.csv.zip"

        file = self._download(url, folder)
        self._validate_download(file)
    ...
    def deliveryBhavcopy(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/products/content/sec_bhavdata_full_{date.strftime('%d%m%Y')}.csv"
        file = self._download(url, folder)
        self._validate_download(file)
        return file
    ...
    def indicesBhavcopy(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/content/indices/ind_close_all_{date.strftime('%d%m%Y')}.csv"
        file = self._download(url, folder)
        self._validate_download(file)
        return file
    ...
    def fnoBhavcopy(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/content/fo/BhavCopy_NSE_FO_0_0_0_{date.strftime('%Y%m%d')}_F_0000.csv.zip"
        file = self._download(url, folder)
        self._validate_download(file)
    ...
    def priceband_report(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/content/equities/sec_list_{date.strftime('%d%m%Y')}.csv"
        file = self._download(url, folder)
        self._validate_download(file)
        return file
    ...
    def pr_bhavcopy(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/archives/equities/bhavcopy/pr/PR{date.strftime('%d%m%y')}.zip"
        file = self._download(url, folder)
        self._validate_download(file)
        return file
    ...
    def cm_mii_security_report(
        ...
        """
        folder = NSE._getPath(folder, isFolder=True) if folder else self.dir
        url = f"{self.archive_url}/content/cm/NSE_CM_security_{date.strftime('%d%m%Y')}.csv.gz"
        file = self._download(url, folder)
        self._validate_download(file)
    ...
    def actions(
        ...
        params.update(self._build_date_params(from_date, to_date))

        return self._req(f"{self.base_url}/corporates-corporateActions", params=params).json()
    ...
    def announcements(
        ...
        return self._req(f"{self.base_url}/corporate-announcements", params=params).json()
    ...
    def gainers(self, data: Dict, count: Optional[int] = None) -> List[Dict]:
        ...
        :rtype: list[dict]
        """
        gainers_list = [dct for dct in data["data"] if dct["pChange"] > 0]
        gainers_list.sort(key=lambda dct: dct["pChange"], reverse=True)
        return gainers_list[:count]
    ...
    def losers(self, data: Dict, count: Optional[int] = None) -> List[Dict]:
        ...
        :rtype: list[dict]
        """
        losers_list = [dct for dct in data["data"] if dct["pChange"] < 0]
        losers_list.sort(key=lambda dct: dct["pChange"])
        return losers_list[:count]
